...
API_URI = "https://api.themoviedb.org/3/movie/upcoming"
API_KEY = "e7e2852e5b4d9cc2fccd5fb9858411d9"

def background_refresh_movie(waiting):
    while True:
        data = rq.get(f"{API_URI}?api_key={API_KEY}").json()
        page = data.get("total_pages")
        
        for page_index in range(1, page+1):

            jmovies = rq.get(f"{API_URI}?api_key={API_KEY}&page={page_index}").json()

            for jmovie in jmovies.get("results"):
                try:
